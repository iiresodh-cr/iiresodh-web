...
export default function NoticiaDetalle() {
  ...
  useEffect(() => {
    ...
    const fetchNoticia = async () => {
      try {
        ...
        let data = null;
        if (!querySnapshot.empty) data = { id: querySnapshot.docs[0].id, ...querySnapshot.docs[0].data() };
        else {
          const docSnap = await getDoc(doc(db, "noticias", id));
          if (docSnap.exists()) data = { id: docSnap.id, ...docSnap.data() };
        }
        if (data) { setNoticia(data); document.title = `${data.titulo} | IIRESODH`; }
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-bold uppercase tracking-widest text-main-blue">Cargando...</div>;
  if (!noticia) return <div className="min-h-screen flex items-center justify-center text-red-500 font-bold">No encontrada</div>;
    ...
    <div className="bg-white min-h-screen flex flex-col font-sans">
      <div className="bg-main-blue text-white py-14 px-6 text-center relative z-20">
        <span className="text-xs font-black text-main-red uppercase tracking-[0.3em] mb-4 block">
          {noticia.fechaPublicacion?.toDate ? noticia.fechaPublicacion.toDate().toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' }) : 'Comunicado Oficial'}
        </span>
            ...
            <div className="px-8 md:px-12 lg:px-16 py-12 md:py-16">
              ...
              <div className="flex flex-col lg:flex-row gap-12 items-start">
                ...
                <div className="w-full lg:w-1/2 shrink-0">
                  <Swiper modules={[Pagination, Autoplay]} pagination={{ clickable: true }} autoplay={{ delay: 5000 }} className="w-full swiper-custom-pagination">
                ...
                <div className="w-full lg:w-1/2">
                  ...
                  <div className="mt-12 pt-8 border-t border-gray-100">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6 text-center lg:text-left">Compartir esta noticia</p>
                    <div className="flex flex-wrap justify-center lg:justify-start gap-3">
                      <a href={shareUrls.whatsapp} target="_blank" rel="noreferrer" className="group flex items-center gap-2 px-4 py-2 rounded-full bg-gray-50 text-gray-500 hover:bg-green-50 hover:text-green-600 transition-all border border-transparent hover:border-green-200">
                  ...
                  <a href={shareUrls.facebook} target="_blank" rel="noreferrer" className="group flex items-center gap-2 px-4 py-2 rounded-full bg-gray-50 text-gray-500 hover:bg-blue-50 hover:text-blue-700 transition-all border border-transparent hover:border-blue-200">
                  ...
                  <a href={shareUrls.twitter} target="_blank" rel="noreferrer" className="group flex items-center gap-2 px-4 py-2 rounded-full bg-gray-50 text-gray-500 hover:bg-black hover:text-white transition-all border border-transparent hover:border-gray-800">
