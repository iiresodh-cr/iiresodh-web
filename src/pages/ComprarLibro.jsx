              ...
              {listaLibros.map((l) => {
                ...
                return (
                  <div key={l.id} className="flex flex-col group h-full">
                    ...
                    <Link to={`/comprar-libro/${l.slug}`} className="block w-full aspect-4/5 rounded-2xl overflow-hidden bg-gray-50 flex items-center justify-center shadow-md group-hover:shadow-xl transition-all duration-300 mb-6 relative">
                      {l.imagenPrincipalUrl ? (
                        <img 
                          src={l.imagenPrincipalUrl} 
                          alt={l.titulo} 
                          className="max-h-full max-w-full object-contain p-4 group-hover:scale-105 transition-transform duration-500 rounded-md shadow-sm" 
                        />
                      ) : (
          ...
          <div className="max-w-7xl mx-auto bg-white overflow-hidden md:rounded-3xl shadow-sm border border-gray-100">
            ...
            <div className="px-8 md:px-12 lg:px-16 py-12 md:py-16 animate-fade-in-up">
              ...
              <div className="flex flex-col md:flex-row gap-10 lg:gap-16 items-start text-center md:text-left">
                ...
                {/* LADO IZQUIERDO: PORTADA CORREGIDA PARA UI/UX */}
                <div className="w-full md:w-5/12 flex justify-center shrink-0">
                  {libro.imagenPrincipalUrl ? (
                    <img 
                      src={libro.imagenPrincipalUrl} 
                      alt={libro.titulo} 
                      // 1. Padding de p-10 en el marco gris para centrar
                      // 2. max-h-[500px] para forzar containment y evitar cortes
                      // 3. object-contain es la clave UX
                      className="max-w-full w-auto object-contain shadow-2xl rounded-xl p-10 bg-gray-50" 
                      style={{ maxHeight: "500px" }} 
                    />
                  ) : (
                    <div className="w-64 h-80 bg-gray-50 rounded-xl shadow-sm border border-gray-200 flex items-center justify-center text-main-blue font-bold p-10 text-center text-sm uppercase leading-tight">
                      {libro.titulo}
                    </div>
                  )}
                </div>
